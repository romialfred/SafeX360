    ...
    return (
        <div className={`p-3 rounded-lg shadow-xl flex flex-col gap-2  ${incidentSummaryCards.borderColor}  border transition-all duration-300 transform hover:shadow-lg hover:scale-[1.01] hover:{safetyIndicators.borderColor}`}>
            <div className="flex gap-4 items-center justify-between">
                ...
                <p className={`text-lg ${incidentSummaryCards.text}`}>{incidentSummaryCards.type}</p>

            </div>
            <div className="flex flex-col gap-0.5">
                <p className={` ${incidentSummaryCards.text}`}>{incidentSummaryCards.incident}</p>
            </div>
            <div className="flex items-baseline gap-2">
                <p className={`${incidentSummaryCards.text} text-lg`}>{incidentSummaryCards.value}</p>
