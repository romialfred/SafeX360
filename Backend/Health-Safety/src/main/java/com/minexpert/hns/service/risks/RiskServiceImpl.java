...
public class RiskServiceImpl implements RiskService {
    ...
    @Override
    public List<RiskDTO> search(String status, Long departmentId, Long ownerId, LocalDate from, LocalDate to, String q) throws HSException {
    ...
    @Override
    public RiskOverviewResponse getOverview(String status, Long departmentId, Long ownerId, LocalDate from, LocalDate to, String q) throws HSException {
    ...
    private OverviewMetrics buildMetrics(List<RiskDTO> items) {
        ...
        int inProgress = (int) items.stream().filter(r -> "IN_PROGRESS".equalsIgnoreCase(s(r.getStatus()))).count();
        int closed = (int) items.stream().filter(r -> "CLOSED".equalsIgnoreCase(s(r.getStatus()))).count();
        LocalDate today = LocalDate.now();
        int overdue = (int) items.stream().filter(r -> r.getReviewDate() != null && r.getReviewDate().isBefore(today)
                && !"CLOSED".equalsIgnoreCase(s(r.getStatus()))).count();
    ...
    private List<TrendPoint> buildTrends(List<RiskDTO> items) {
        ...
        Map<String, Long> totalByMonth = items.stream()
                ...
                .collect(Collectors.groupingBy(r -> r.getCreatedAt().format(fmt), Collectors.counting()));
    ...
    private static String normalize(String v) { return v == null || v.isBlank() ? null : v; }
}
