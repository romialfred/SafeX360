...
public class ServiceTokenVerifier {
    ...
    public ServiceTokenVerifier(
            @Value("${SAFEX_GATEWAY_HNS_TOKEN_KEY:}") String gatewayMaterial,
            @Value("${SAFEX_HRMS_HNS_TOKEN_KEY:}") String hrmsMaterial,
            Environment environment) {
        this(gatewayMaterial, hrmsMaterial, environment, Clock.systemUTC());
    }

    ServiceTokenVerifier(String gatewayMaterial, String hrmsMaterial, Environment environment, Clock clock) {
        boolean local = environment.acceptsProfiles(Profiles.of("dev", "test", "local"));
        gatewayMaterial = material(gatewayMaterial, local, "gateway-hns");
        hrmsMaterial = material(hrmsMaterial, local, "hrms-hns");
        ...
        this.issuerKeys = Map.of(
                "safex-gateway", key(gatewayMaterial),
                "safex-hrms", key(hrmsMaterial));
        this.clock = clock;
    }
    ...
    public ServiceIdentity verify(String compactToken) {
        ...
        for (Map.Entry<String, SecretKey> candidate : issuerKeys.entrySet()) {
            try {
                ...
                if (candidate.getKey().equals(parsed.getIssuer())) {
                    claims = parsed;
                    matchedIssuer = candidate.getKey();
                    break;
                }
        ...
        if (!"safex-hns".equals(claims.getAudience())) throw new ServiceTokenException("SERVICE_TOKEN_AUDIENCE_INVALID");
        ...
        return new ServiceIdentity(matchedIssuer, claims.getAudience(), scopes, jti);
    }
