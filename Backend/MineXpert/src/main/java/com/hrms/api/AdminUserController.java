...
public class AdminUserController {
    ...
    public static class CreateUserRequest {
        ...
        /** Role predefini cote frontend (SYSTEM_ADMINISTRATOR, HSE_COORDINATOR, AUDITOR, etc.). */
        private String role;
        /** Liste des moduleIds autorises (CSV, voir ModuleConfig.tsx). Peut etre null si role suffit. */
        private String allowedModules;
        /** Optionnel : companyId pour multi-tenant. */
    ...
    public ResponseEntity<CreateUserResponse> createUser(@RequestBody CreateUserRequest req) throws HRMSException {
        ...
        // ─── 3. Generation MDP temporaire fort ───
        ...
        account.setFirstLogin(true); // force changement au premier login
        account.setStatus("ACTIVE");
        account.setStartDate(LocalDateTime.now());

        Account saved = accountRepository.save(account);
        LOG.info("Account created: id={}, login={}, role={}", saved.getId(), saved.getLogin(), saved.getRole());

        // ─── 5. Init du PermissionProfile cote HSE (best-effort, ne fait pas echouer) ───
        boolean permissionsInit = initPermissionsHSE(saved.getId(), req.getRole(), req.getAllowedModules());
        if (!permissionsInit) {
            LOG.warn("Permission profile init failed for accountId={}, role={} - account created without HSE perms", saved.getId(), req.getRole());
        }

        // ─── 6. Envoi email avec MDP en clair (one-time) ───
        boolean emailSent = sendCreationEmail(req.getEmail(), req.getName(), req.getLogin(), tempPassword);

        return new ResponseEntity<>(new CreateUserResponse(
                saved.getId(),
                saved.getLogin(),
                saved.getEmail(),
                emailSent ? null : tempPassword, // si email OK on ne renvoie pas le MDP, sinon on le renvoie pour copie manuelle
                emailSent,
                emailSent
                    ? "Compte cree. Email envoye avec mot de passe temporaire."
                    : "Compte cree. Email NON envoye - copiez le mot de passe temporaire ci-dessous."
        ), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<ResetPasswordResponse> resetPassword(@PathVariable Long id) throws HRMSException {
        ...
        return new ResponseEntity<>(new ResetPasswordResponse(
                ...
                emailSent,
                emailSent
                    ? "MDP reinitialise. Email envoye."
                    : "MDP reinitialise. Email NON envoye - copiez le mot de passe ci-dessous."
        ), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ToggleStatusResponse> toggleStatus(@PathVariable Long id) throws HRMSException {
        Account account = accountRepository.findById(id)
                .orElseThrow(() -> new HRMSException("ACCOUNT_NOT_FOUND"));
